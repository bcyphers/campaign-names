...
Bubbles = () ->
  ...
  rValue = (d) -> Math.sqrt(parseInt(d.count))
  ...
  updateLabels = () ->
    ...
    #  styling divs
    label
      .style("font-size", (d) -> Math.max(8, rScale(rValue(d) / 4)) + "px")
...
$ ->
  # load our data
  d3.csv("data/hillary-names.csv", (data) ->
    plotData("#vis-hillary", data, Bubbles()))

  d3.csv("data/trump-names.csv", (data) ->
    plotData("#vis-trump", data, Bubbles()))
